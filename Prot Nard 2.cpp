...
bool go = false; // Проверка на правильность хода
...
int max = 0; // Поиск максимальной координаты по Y
int min = 0; // Поиск минимальной координаты по Y
...
void Draw() // Функция отрисовки игрового поля и нард
{
	system("cls");
...
void Step() { // Функция проверки на правильность хода
	if (fromX - whereX == cube1 * 3) {
		go = true;
	}
	else if (fromX - whereX == cube2 * 3) {
		go = true;
	}
	else {
		go = false;
...
void Step1() { // Функция проверки на правильность хода
	if (fromX + (cube1 * 3) == whereX) {
		go = true;
	}
	else if (fromX + (cube2 * 3) == whereX) {
		go = true;
	}
	else {
		go = false;
...
void Motion()
{
	...
	else
	{
		...
		for (int j = 0; j < 12; j++)
		{
			if (blakX[j] == fromX && blakY[j] == fromY && go == true)
			{
				blakX[j] = whereX;
				blakY[j] = whereY;
				coll--;
